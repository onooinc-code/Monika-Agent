...
const EXAMPLE_CODE = `// This is a self-contained component.
// CSS is embedded in a <style> tag to avoid external dependencies.
const Button = () => {
  return (
    <div>
      <style>{\`
        .button-send-wrapper .button {
          --primary: #ff5569;
          --neutral-1: #f7f8f7;
          --neutral-2: #e7e7e7;
          --radius: 14px;
        ...
        .button-send-wrapper .button:hover::after {
          transform: scale(1.05, 1.1);
          box-shadow: inset 0 -1px 3px 0 rgba(255, 255, 255, 1);
        }
        .button-send-wrapper .button::before {
          content: "";
        ...
        .button-send-wrapper .state p { display: flex; align-items: center; justify-content: center; }
        .button-send-wrapper .state .icon { position: absolute; left: 0; top: 0; bottom: 0; margin: auto; transform: scale(1.25); transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; }
        .button-send-wrapper .state .icon svg { overflow: visible; }
        .button-send-wrapper .outline { position: absolute; border-radius: inherit; overflow: hidden; z-index: 1; opacity: 0; transition: opacity 0.4s ease; inset: -2px -3.5px; }
        .button-send-wrapper .outline::before { content: ""; position: absolute; inset: -100%; background: conic-gradient(from 180deg, transparent 60%, white 80%, transparent 100%); animation: spin 2s linear infinite; animation-play-state: paused; }
        @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
        .button-send-wrapper .button:hover .outline { opacity: 1; }
        .button-send-wrapper .button:hover .outline::before { animation-play-state: running; }
        .button-send-wrapper .state p span { display: block; opacity: 0; animation: slideDown 0.8s ease forwards calc(var(--i) * 0.03s); }
        .button-send-wrapper .button:hover p span { opacity: 1; animation: wave 0.5s ease forwards calc(var(--i) * 0.02s); }
        .button-send-wrapper .button:focus p span { opacity: 1; animation: disapear 0.6s ease forwards calc(var(--i) * 0.03s); }
        ...
        .button-send-wrapper .state--default .icon svg { animation: land 0.6s ease forwards; }
        .button-send-wrapper .button:hover .state--default .icon { transform: rotate(45deg) scale(1.25); }
        .button-send-wrapper .button:focus .state--default svg { animation: takeOff 0.8s linear forwards; }
        .button-send-wrapper .button:focus .state--default .icon { transform: rotate(0) scale(1.25); }
        @keyframes takeOff { 0% { opacity: 1; } 60% { opacity: 1; transform: translateX(70px) rotate(45deg) scale(2); } 100% { opacity: 0; transform: translateX(160px) rotate(45deg) scale(0); } }
        @keyframes land { 0% { transform: translateX(-60px) translateY(30px) rotate(-50deg) scale(2); opacity: 0; filter: blur(3px); } 100% { transform: translateX(0) translateY(0) rotate(0); opacity: 1; filter: blur(0); } }
        .button-send-wrapper .state--default .icon:before { content: ""; position: absolute; top: 50%; height: 2px; width: 0; left: -5px; background: linear-gradient(to right, transparent, rgba(0, 0, 0, 0.5)); }
        .button-send-wrapper .button:focus .state--default .icon:before { animation: contrail 0.8s linear forwards; }
        @keyframes contrail { 0% { width: 0; opacity: 1; } 8% { width: 15px; } 60% { opacity: 0.7; width: 80px; } 100% { opacity: 0; width: 160px; } }
        .button-send-wrapper .state { padding-left: 29px; z-index: 2; display: flex; position: relative; }
        .button-send-wrapper .state--default span:nth-child(4) { margin-right: 5px; }
        .button-send-wrapper .state--sent { display: none; }
        .button-send-wrapper .state--sent svg { transform: scale(1.25); margin-right: 8px; }
        .button-send-wrapper .button:focus .state--default { position: absolute; }
        .button-send-wrapper .button:focus .state--sent { display: flex; }
        .button-send-wrapper .button:focus .state--sent span { opacity: 0; animation: slideDown 0.8s ease forwards calc(var(--i) * 0.2s); }
        .button-send-wrapper .button:focus .state--sent .icon svg { opacity: 0; animation: appear 1.2s ease forwards 0.8s; }
        @keyframes appear { 0% { opacity: 0; transform: scale(4) rotate(-40deg); color: var(--primary); filter: blur(4px); } 30% { opacity: 1; transform: scale(0.6); filter: blur(1px); } 50% { opacity: 1; transform: scale(1.2); filter: blur(0); } 100% { opacity: 1; transform: scale(1); } }
      \`}</style>
      <div className="button-send-wrapper">
        <button className="button">
          ...
          <div className="state state--default">
            ...
            <p>
              ...
              <span style={{'--i': 4}}>M</span><span style={{'--i': 5}}>e</span><span style={{'--i': 6}}>s</span><span style={{'--i': 7}}>s</span><span style={{'--i': 8}}>a</span><span style={{'--i': 9}}>g</span><span style={{'--i': 10}}>e</span>
            </p>
          </div>
          <div className="state state--sent">
            <div className="icon">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 25" height="1em" width="1em" strokeWidth="0.5px" stroke="black">
                <g style={{filter: 'url(#shadow)'}}>
...
export const AddComponentModal: React.FC<AddComponentModalProps> = ({ isOpen, onClose, onAddComponent }) => {
    const [name, setName] = useState('SendMessageButton');
    ...
    const handleSubmit = () => {
        ...
        // Check if component name is a valid JS identifier
         ...
         <div className="fixed inset-0 flex justify-center items-center z-50 p-4 modal-overlay open" onClick={onClose}>
            <div className="glass-pane rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col modal-content shadow-purple-500/20" onClick={e => e.stopPropagation()}>
                ...
                <footer className="p-6 border-t border-white/10 flex justify-end gap-3">
                    ...
                    <button onClick={handleSubmit} className="bg-purple-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-purple-500 transition-all transform hover:scale-105 neon-glow-purple">Add Component</button>
                </footer>
            </div>
        </div>
    );
